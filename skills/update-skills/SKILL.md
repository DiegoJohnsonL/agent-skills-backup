...
2. **Update.** Run `npx -y skills@latest update --global --yes` from `~/.agents`. The CLI overwrites each refreshed `SKILL.md` wholesale and drops any frontmatter you added by hand. Done when the update finishes.

   **animations.dev skills.** The paid animations.dev skills (`animate`, `css-animations`, `motion-react`, `motion-brief`, `pick-ui-library`, `find-animation-opportunities`, `animation-prototype`, `review-animations`, `improve-animations`, `animation-performance`, `animation-accessibility`, `animation-vocabulary`, `gesture-ui`, `scroll-animations`, `debug-animation`) have no GitHub source, so the CLI never sees them. Refresh them with `~/.agents/scripts/install-animationsdev.sh`, which reads the token from `~/.agents/secrets/animationsdev-token`, stages the vendor installer in a temp dir, copies each skill into the library, and relinks every agent dir. It renames upstream `prototype` to `animation-prototype` because the mattpocock `prototype` skill owns that name. Done when the script prints the installed list.
